...
let rec execute_instruction instrs stack =
  ...
  | Bytecode.QUOTE v :: rest ->
    let rec quote_value_to_stack v =
      match v with
      | Bytecode.VNum n -> SNum n
      | Bytecode.VAtom s -> SAtom s
      | Bytecode.VList vs -> SList (List.map quote_value_to_stack vs)
    in
    let stack_v = quote_value_to_stack v in
  ...
  | Bytecode.CALL_BUILTIN "+" :: rest ->
    let rec sum_all acc s = match s with
      ...
      | _ -> raise (Comptime_state.Runtime_error "Type error: + expoects numbers")
    in
  ...
  | Bytecode.CALL_BUILTIN "<" :: rest ->
    (match stack with
    | [SNum a; SNum b] -> execute_instruction rest [if b < a then SNum 1 else SNum 0]
    | _ -> raise (Comptime_state.Runtime_error "Type error: < requires numbers"))
  | Bytecode.CALL_BUILTIN ">" :: rest ->
    (match stack with
    | [SNum a; SNum b] -> execute_instruction rest [if b > a then SNum 1 else SNum 0]
